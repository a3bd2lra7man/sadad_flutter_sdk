...
class SadadFlutterSdkPlugin : FlutterPlugin, MethodCallHandler, ActivityAware {
    ...
    override fun onMethodCall(@NonNull call: MethodCall, @NonNull result: Result) {
        if (call.method == SADAD_CRATE_TRANSACTION) {
            val sadadTransaction = ArgumentsMapper().toSadadTransaction(call.arguments)
            startSadadTransaction(sadadTransaction, result)
        }
    }

    private fun startSadadTransaction(sadadTransaction: SadadTransaction, result: Result) {
