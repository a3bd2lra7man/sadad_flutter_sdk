            ...
            if statusCode == 200{
                if let transectionStatusID = transferResponse.value(forKey: "transactionstatus") as? Int{
                    if transectionStatusID != transectionSuccessfull{
                        let sadadTransactionResult = SadadTransactionResult(SadadTransactionResultStatus.TransactionFailed,"Transaction Failed",nil)
                ...
                if let strTransectionID = transferResponse.value(forKey: "transactionnumber") as? String{
                    ...
                    //                        self.postPaymentTransactionResultThenRefreshWalletData(transactionNumber: strTransectionID);
                }
                if let strTransectionID = transferResponse.value(forKey: "invoicenumber") as? String{
                    let sadadTransactionResult = SadadTransactionResult(SadadTransactionResultStatus.Success,"Success",strTransectionID)
                    self.onSuccess(sadadTransactionResult);
                    //                        self.postPaymentTransactionResultThenRefreshWalletData(transactionNumber: strTransectionID);
                }
